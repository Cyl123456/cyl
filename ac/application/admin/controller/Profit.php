...
class Profit extends Common {
	...
	public function wal() {
		...
		foreach ($list as $k => $v) {
			if(in_array($v['wal_type'],[2,3,4,7,8])){
	...
	public function sal() {
		...
		if (input('get.type') != "") {
			$this->map[] = ['sal_type', '=', input('get.type')];
		}
		if (input('get.a') == 1) {
			$list = model("ProMsc")->where($this->map)->select();
			// $url = action('Excel/user', ['list' => $list]);
			$bb = env('ROOT_PATH') . "public\sal.xlsx";
			if (file_exists($bb)) {
				$aa = unlink($bb);
			}
			$spreadsheet = new Spreadsheet();
			...
			foreach ($list as $k => $v) {
				$sheet->setCellValue('A' . $i, $v['us_text'])
					->setCellValue('B' . $i, $v['us_name'])
					->setCellValue('C' . $i, $v['sal_num'])
					->setCellValue('D' . $i, $v['sal_note'])
					->setCellValue('E' . $i, $v['sal_add_time']);
				$i++;
			}
			$writer = new Xlsx($spreadsheet);
			$writer->save('sal.xlsx');
			return "http://" . $_SERVER['HTTP_HOST'] . "/sal.xlsx";
		}
		if(input('size')){
			$this->size = input('size');
		}
		$list = model('ProSal')->chaxun($this->map, $this->order, $this->size);
		foreach ($list as $k => $v) {
			if(in_array($v['sal_type'],[2,6,9])){
				$list[$k]['sal_num'] = '-'.$v['sal_num'];
			}
		}
		$num = model("ProSal")->where($this->map)->sum('sal_num');
		$this->assign(array(
			'list' => $list,
			'num'=>$num,
		));
		...
		if(is_post()){
			...
			if ($rst) {
				if (input('post.status') == 2) {
					model("User")::usMscChange($info['us_id'],$info['tx_num'],11);
				}
				$this->success('已审核');
			} else {
				$this->error('操作失败');
	...
	public function jd() {
		...
		$list = model('ProJd')->chaxun($this->map, $this->order, $this->size);
		foreach ($list as $k => $v) {
			$dd = Db::name('sto_order_detail')->where('us_id',$v['id'])->where('detail_add_time','month')->find();
			if($dd){
				$list[$k]['gouwu'] = '已购物';
			}else{
				$list[$k]['gouwu'] = '未购物';
		...
		$num = model('ProJd')->where($this->map)->sum('jd_num');
		$this->assign(array(
			'list' => $list,
			'num' => $num,
		));
	...
	public function jdCheck() {
		if(is_post()){
			$da = input('post.');
			$id = input('post.id');
			$info = model('ProJd')->get($id);
			$rst = model('ProJd')->xiugai(['jd_status' => input('post.status')], ['id' => input('post.id')]);
			if ($rst) {
				if (input('post.status') == 1) {
					model('User')::usMscChange($info['us_id'],$info['jd_num'],$info['jd_type'],$info['jd_from']);
				}
				$this->success('已审核');
			} else {
				$this->error('操作失败');
