...
class Baod extends Base 
{
    ...
    public function index(){
        if(is_post()){
            ...
            $list = model('StoProd')->where($this->map)->order($this->order)->limit(3)->select();
            foreach ($list as $k => $v) {
                $list[$k]['pic'] = explode(',',$v['prod_pic'])[0];
            }  
            $this->msg($list);
        }else{
            $this->s_msg('get');
    ...
    public function buy(){
        if(is_post()){
            ...
            if($rel){
                //扣除用户茶币
                model('User')::usWalChange($this->user['id'],$all_money,8);
                //更新产品库存销量
                model('Stoprod')
                ->where(['id' => $prod['id']])
                ->dec('prod_res', 1)
                ...
                $brr = [
                    'id' => $this->user['id'],
                    'us_rel' => $this->user['us_rel']+$all_money,
                ];
                Db::name('user')->update($brr);
                //复投见点
                model("User")->point($this->user['id']);
                
                model('User')->yeji($this->user['id'],$all_money,13);
  ...
  //               $peng_type = 5; //对碰奖励类型
  //           }else{
  //               // $all_money =  $prod['prod_price']*cache('setting')['recover_zhe']/10;
  //               $all_money =  770;
  //               $wal_type = 8;
  //               $peng_type = 13;
  //           }
