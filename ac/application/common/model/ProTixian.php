...
class ProTixian extends Model {
	...
	public function tianjia($arr) {
		...
		// 	'us_id'       =>  $id,
		// 	'tx_num' 	  =>  $num,
		// 	'tx_add_time' =>  date('Y-m-d H:i:s'),
		// ];
		$arr['tx_recharge'] = cache('setting')['cal_recharge']."%";
		$arr['tx_shidao'] = $arr['tx_num']-$arr['tx_num']*cache('setting')['cal_recharge']/100;
		$arr['tx_add_time'] = date('Y-m-d H:i:s');
		$rel = $this->insertGetId($arr);
		if ($rel) {
			User::usMscChange($arr['us_id'],$arr['tx_num'],9);
			return [
				'code' => 1,
				'msg' => '提现成功,等待后台审核',
			];
